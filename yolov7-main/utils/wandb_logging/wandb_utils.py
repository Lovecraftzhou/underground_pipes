...
class WandbLogger():
    ...
    def check_and_upload_dataset(self, opt):
        assert wandb, 'Install wandb to upload pipe'
        check_dataset(self.data_dict)
        config_path = self.log_dataset_artifact(opt.data,
                                                opt.single_cls,
                                                'YOLOR' if opt.project == 'runs/train' else Path(opt.project).stem)
        print("Created pipe config file ", config_path)
    ...
    def download_dataset_artifact(self, path, alias):
        if isinstance(path, str) and path.startswith(WANDB_ARTIFACT_PREFIX):
            ...
            assert dataset_artifact is not None, "'Error: W&B pipe artifact doesn\'t exist'"
    ...
    def create_dataset_table(self, dataset, class_to_id, name='pipe'):
        # TODO: Explore multiprocessing to slpit this loop parallely| This is essential for speeding up the the logging
        artifact = wandb.Artifact(name=name, type="pipe")
