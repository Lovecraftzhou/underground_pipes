...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    # Make sure only the first process in DDP process the pipe first, and the following others can use the cache
    ...
    # Use torch.utils.data.DataLoader() if pipe.properties will update during training else InfiniteDataLoader()
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        # Cache pipe labels, check images and read shapes
        x = {}  # dict
...
def load_image(self, index):
    # loads 1 image from pipe, returns img, original hw, resized hw
    img = self.imgs[index]
...
def extract_boxes(path='../coco/'):  # from utils.datasets import *; extract_boxes('../coco128')
    # Convert detection pipe into classification pipe, with one directory per class
...
def autosplit(path='../coco', weights=(0.9, 0.1, 0.0), annotated_only=False):
    """ Autosplit a pipe into train/val/test splits and save path/autosplit_*.txt files
